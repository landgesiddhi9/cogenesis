...
const SortDropdown = ({ options, selectedId, onSelect }: SortDropdownProps) => {
  ...
  const closeTimerRef = useRef<ReturnType<typeof setTimeout> | undefined>(undefined); const dropdownRef = useRef<HTMLDivElement>(null);
  ...
  return (
    <div ref={dropdownRef} className="relative inline-block">
      ...
      {(isOpen || closing) && (
        <div
          className={`absolute top-full right-0 mt-2 w-56 bg-ivory border border-stone/15 rounded-lg shadow-lg overflow-hidden z-40 transition-all ${closing
          ...
          <div className="py-2">
            {options.map((option) => (
              <button
                ...
                className={`w-full text-left px-4 py-3 text-sm transition-all duration-150 flex items-center justify-between ${option.id === selectedId
